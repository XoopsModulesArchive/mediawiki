...
'<head><title>test</title></head><body>'.$text.'</body></html>';
    ...
    function doTableStuff ( $t )
    {
        ...
        foreach ($t AS $k => $x) {
            ...
            } elseif ( count ( $td ) == 0 ) { } # Don't do any of the following
            ...
            } elseif ( '|-' == substr ( $x , 0 , 2 ) ) { # Allows for |---------------
                $x = substr ( $x , 1 ) ;
            ...
            } elseif ( '|' == $fc || '!' == $fc || '|+' == substr ( $x , 0 , 2 ) ) { # Caption
                # $x is a table row
                if ( '|+' == substr ( $x , 0 , 2 ) ) {
                    $fc = '+' ;
                    $x = substr ( $x , 1 ) ;
                ...
                if (($i % 2) == 0) {
                    ...
                    if (strlen($r) == 2) {
                        if ($state == 'i') {
                            ...
                            $output .= '<b><i>' . $buffer . '</i>';
                            $state = 'b';
                        } else { # $state can be 'b' or '' $output .= '<i>'; $state .= 'i';
                        }
                    } elseif (strlen($r) == 3) {
                        if ($state == 'b') {
                            $output .= '</b>';
                            ...
                            $output .= '<i><b>' . $buffer . '</b>';
                            $state = 'i';
                        } else { # $state can be 'i' or '' $output .= '<b>'; $state .= 'b';
                        }
                    } elseif (strlen($r) == 5) {
                        if ($state == 'b') {
                            $output .= '</b><i>';
    ...
    /* private */ function openList( $char ) {
        ...
        if ('*' == $char) { $result .= '<ul><li>'; } elseif ('#' == $char) { $result .= '<ol><li>'; } elseif (':' == $char) { $result .= '<dl><dd>'; } elseif (';' == $char) {
            $result .= '<dl><dt>';
            $this->mDTopen = true;
        } else { $result = '<!-- ERR 1 -->'; }
    ...
    /* private */ function nextItem( $char ) {
        if ('*' == $char || '#' == $char) { return '</li><li>'; } elseif (':' == $char || ';' == $char) {
            $close = '</dd>';
    ...
    /* private */ function closeList( $char ) {
        if ('*' == $char) { $text = '</li></ul>'; } elseif ('#' == $char) { $text = '</li></ol>'; } elseif (':' == $char) {
            if ($this->mDTopen) {
                $this->mDTopen = false;
                $text = '</dt></dl>';
            } else {
    ...
    function argSubstitution( $matches )
    {
        ...
        if ( array_key_exists( $arg, $inputArgs ) ) {
            $text = $inputArgs[$arg];
        } elseif ($this->mOutputType == OT_HTML && null != $matches['parts'] && count($matches['parts']) > 0) {
